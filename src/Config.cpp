...
void Config::set_servers(std::map<int, _map> &raw_servers) {
  ...
  for (std::map<int, _map>::iterator it = raw_servers.begin();
       it != raw_servers.end(); it++) {
    try {
      ...
      for (_map::iterator map_it = it->second.begin();
        ...
        if (_server_setters.find(map_it->first) == _server_setters.end()) {
          if (Utils::valid_server_setting(map_it->first) == false) {
            Utils::config_error_on_line(map_it->second.second,
                                        "'" + map_it->first +
...
void Config::abort(Server *&new_server) {
  ...
  for (std::vector<Server *>::iterator server = _servers.begin();
       server != _servers.end(); server++) {
    delete *server;
  }
  for (std::vector<LocationInfo *>::iterator location = _locations.begin();
       location != _locations.end(); location++) {
    delete *location;
  }
  throw std::runtime_error("Aborting initialization, please double-check your "
                           "server configuration.\n");
}

void Config::add_location(LocationInfo *&new_location, Server *new_server,
                          bool add_to_server) {
...
Config::location_setter_map::iterator
Config::initialize_location(int line_count, const std::string &key,
...
void Config::configure_locations(const _map &server, Server *&new_server) {
  ...
  for (_map::const_iterator it = server.begin(); it != server.end(); it++) {
    ...
    location_setter_map::iterator setter =
        initialize_location(it->second.second, it->first, name, new_location);
    ...
    if (setter != _location_setters.end()) {
      ...
      Utils::config_error_on_line(it->second.second,
                                  "'" + it->first +
                                      "' is not a valid location setting");
    }
  }
  add_location(new_location, new_server, true);
}

void Config::initialize_cgi(LocationInfo *&new_location,
...
Config::location_setter_map::iterator
Config::configure_cgi(int line_count, const std::string key,
                      LocationInfo *&new_location) {
  std::string cgi_identifier = Utils::extract_cgi_identifier(key);
  if (cgi_identifier.empty() == true) {
    Utils::config_error_on_line(line_count,
                                "missing CGI identifier in location '" + key +
                                    "', location will not be initialized");
...
void Config::configure_host(_map &server, Server *&new_server) {
  if (server.find("host") == server.end() or
      server["host"].first.empty() == true) {
    Utils::config_error_on_line(-1, "missing host in server '" +
                                        new_server->get_server_name()[0] +
                                        "'.");
  }
  ...
  if (inet_pton(AF_INET, host.c_str(), &ip_address) != 1) {
    Utils::config_error_on_line(server["host"].second,
...
void Config::configure_server_names(_map &server, Server *&new_server) {
  if (server.find("server_name") == server.end() or
      server["server_name"].first.empty() == true) {
    server["server_name"].first.push_back("default.com");
  }

  std::vector<std::string> new_server_names = server["server_name"].first;

  for (std::vector<std::string>::const_iterator it = new_server_names.begin();
       it != new_server_names.end(); it++) {
    if (std::find(_server_names.begin(), _server_names.end(), *it) !=
        _server_names.end()) {
      Utils::config_error_on_line(server["server_name"].second,
                                  "on server: '" + *it +
                                      "': name already taken.");
    }
  }

  _server_names.insert(_server_names.begin(), new_server_names.begin(),
                       new_server_names.end());

  new_server->set_server_name(new_server_names);
}

void Config::configure_port(_map &server, Server *&new_server) {
  if (server.find("port") == server.end() or
      server["port"].first.empty() == true) {
    Utils::config_error_on_line(-1, "missing port in server '" +
                                        new_server->get_server_name()[0] +
                                        "'.");
  }
...
void Config::initialize_server_setters() {
  ...
  _server_setters["client_max_body_size"] =
      &Setters::configure_client_max_body_size;
